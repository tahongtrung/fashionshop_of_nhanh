...
if(isset($_GET['mf'])){
    $mf = $_GET['mf'];
    $sql_mf = "SELECT pd.id,pd.category_id,pd.name,mf.mf_name,pd.img,pd.price FROM manufacture as mf JOIN product as pd ON pd.manufacture = mf.id WHERE mf.mf_code = '$mf'";
    $result_mf = mysqli_query($link,$sql_mf); 
}
?>
<!DOCTYPE html>
<html>
        ...
        <div class="wrapper catelogue">
            ...
            <div style="width:80%;float:right;height:auto;">
                <div class="con-product" style="width:100%;height:1100px;">
                    <ul>
                        <?php while($row=mysqli_fetch_assoc($result_mf)){  
                        ?>
