...
require_once("pdProduct.php");
    if(isset($_POST['checkSize'])){
        $check = $_POST['checkSize'];
       $sql_size = "SELECT pd.id,pd.category_id,pd.name,mf.mf_name,pd.img,pd.price FROM manufacture as mf JOIN product as pd ON pd.manufacture = mf.id JOIN size as sz ON pd.size_id = sz.id WHERE sz.name = '$check'";
